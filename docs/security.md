...
BridgeDesk exposes local coding capabilities over MCP. Treat it as remote access
...
BridgeDesk only opens workspaces under configured roots.
...
`bridgedesk init` generates an Owner password and stores it in:

```text
~/.bridgedesk/auth.json
```

When an MCP client connects, BridgeDesk shows an approval page. Enter the Owner
...
BridgeDesk needs `BRIDGEDESK_PUBLIC_BASE_URL` so MCP clients can discover OAuth
...
Do not include `/mcp` in `BRIDGEDESK_PUBLIC_BASE_URL`.

By default, BridgeDesk derives allowed Host headers from the local host and public
URL. Use `BRIDGEDESK_ALLOWED_HOSTS=*` only for intentional local debugging.

## Tunnels

BridgeDesk does not manage tunnels. Your tunnel or reverse proxy should point to:
...
protection in front of public tunnels. BridgeDesk OAuth still protects the MCP
...
Filesystem path containment applies to BridgeDesk file tools. Shell commands run
...
By default, BridgeDesk logs requests and tool calls. Shell command previews are
disabled unless `BRIDGEDESK_LOG_SHELL_COMMANDS=1`.
