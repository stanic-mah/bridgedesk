...
const root = await mkdtemp(join(tmpdir(), "bridgedesk-workspace-test-"));
...
try {
  ...
  const config = loadConfig({
    BRIDGEDESK_ALLOWED_ROOTS: root,
    BRIDGEDESK_WORKTREE_ROOT: join(root, ".bridgedesk", "worktrees"),
  ...
  await git(gitRoot, ["config", "user.email", "bridgedesk@example.com"]);
  await git(gitRoot, ["config", "user.name", "BridgeDesk Test"]);
  ...
  if (platform() !== "win32") {
    ...
    const aliasConfig = loadConfig({
      BRIDGEDESK_ALLOWED_ROOTS: aliasRoot,
      BRIDGEDESK_WORKTREE_ROOT: join(aliasRoot, ".bridgedesk", "alias-worktrees"),
      BRIDGEDESK_AGENT_DIR: agentDir,
      BRIDGEDESK_OAUTH_OWNER_TOKEN: "test-owner-token-that-is-long-enough",
      PORT: "1",
    });
