...
const root = await mkdtemp(join(tmpdir(), "bridgedesk-oauth-test-"));
const oauthConfig = {
  ownerToken: "test-owner-token-that-is-long-enough",
  accessTokenTtlSeconds: 3600,
  refreshTokenTtlSeconds: 2592000,
  scopes: ["bridgedesk", "offline_access"],
  allowedRedirectHosts: [".chatgpt.com", ".openai.com"],
  clientStorePath: join(root, "oauth-clients.json"),
};
...
async function testDatabaseConfiguration(stateDir: string): Promise<void> {
  ...
  try {
    ...
    const migrations = database.sqlite
      .prepare("select version, name from bridgedesk_schema_migrations order by version")
      .all();
...
function testExpiredTokenCleanup(stateDir: string): void {
  ...
  store.saveTokenPair({
    ...
    accessToken: { clientId: client.client_id, scopes: ["bridgedesk"], expiresAt: expiredAt },
    refreshTokenHash: "expired-refresh-hash",
    refreshToken: { clientId: client.client_id, scopes: ["bridgedesk"], expiresAt: expiredAt },
  });
  store.close();
      ...
      store.saveTokenPair(
        {
          ...
          accessToken: { clientId: client.client_id, scopes: ["bridgedesk"], expiresAt },
          refreshTokenHash: "new-refresh-hash",
          refreshToken: { clientId: client.client_id, scopes: ["bridgedesk"], expiresAt },
        },
        "old-refresh-hash",
      ),
      true,
...
async function testProviderRestartRotationAndRevocation(stateDir: string): Promise<void> {
  ...
  try {
    ...
    await assert.rejects(
      secondProvider.exchangeRefreshToken(client, issued.refresh_token, ["bridgedesk"], mcpUrl),
      InvalidGrantError,
    );
    ...
    await assert.rejects(
      secondProvider.exchangeRefreshToken(client, refreshed.refresh_token, ["bridgedesk"], mcpUrl),
      InvalidGrantError,
    );
  } finally {
