...
async function ensureConfigured(): Promise<void> {
  ...
  if (process.env.BRIDGEDESK_OAUTH_OWNER_TOKEN) return;

  if (!input.isTTY || !output.isTTY) {
    throw new Error(
      [
        "BridgeDesk is not configured and this terminal is non-interactive.",
        "",
        "Run:",
        "  bridgedesk init",
        "",
        "Or provide BRIDGEDESK_OAUTH_OWNER_TOKEN and BRIDGEDESK_ALLOWED_ROOTS.",
      ].join("\n"),
    );
  }
...
async function runInit({ force }: { force: boolean }): Promise<void> {
  ...
  if (!force && files.configExists && files.authExists) {
    prompts.log.info(`BridgeDesk is already configured at ${files.dir}`);
    prompts.log.info("Run `bridgedesk init --force` to update it.");
    return;
  }

  try {
    prompts.intro("BridgeDesk setup");
    ...
    const portAnswer = await textPrompt({
      message: `Which local port should BridgeDesk use? Press Enter to use ${defaultPort}`,
    ...
    prompts.note(
      [
        "BridgeDesk needs a public base URL so ChatGPT or Claude can reach this MCP server.",
    ...
    const config: BridgeDeskUserConfig = {
    ...
    const configPath = writeBridgeDeskConfig(config);
    const authPath = writeBridgeDeskAuth(auth);
    ...
    prompts.note(lines.join("\n"), "BridgeDesk configured");
    prompts.note(
      [
        `Owner password: ${auth.ownerToken}`,
        "Use this when ChatGPT or Claude asks you to approve BridgeDesk access.",
        `Stored at: ${authPath}`,
      ].join("\n"),
      "Owner password",
    );
    prompts.outro("Run `bridgedesk serve` to start the MCP server.");
  } catch (error) {
...
async function serve(): Promise<void> {
  ...
  const httpServer = app.listen(config.port, config.host, () => {
    console.log(`bridgedesk listening on http://${config.host}:${config.port}/mcp`);
    console.log(`public base url: ${config.publicBaseUrl}`);
    console.log(`allowed roots: ${config.allowedRoots.join(", ")}`);
    console.log(`allowed hosts: ${config.allowedHosts.join(", ")}`);
    if (config.allowedHosts.includes("*")) {
      console.warn("warning: Host header allowlist is disabled because BRIDGEDESK_ALLOWED_HOSTS=*");
    }
...
function runConfigCommand(args: string[]): void {
  ...
  if (key !== "publicBaseUrl") {
    throw new Error("Only `bridgedesk config set publicBaseUrl <url|null>` is supported right now.");
  }
...
function printHelp(): void {
  console.log(
    [
      ...
      "",
      "Usage:",
      "  bridgedesk                 Run first-time setup if needed, then start the server",
      "  bridgedesk serve           Start the server",
      "  bridgedesk init            Create or update ~/.bridgedesk/config.json and auth.json",
      "  bridgedesk doctor          Show config, runtime, and native dependency status",
      "  bridgedesk config get      Print persisted config",
      "  bridgedesk config set publicBaseUrl <url|null>",
      "",
      "For temporary tunnels:",
      "  BRIDGEDESK_PUBLIC_BASE_URL=https://example.trycloudflare.com bridgedesk serve",
    ].join("\n"),
  );
}
...
function assertSupportedNode(): void {
  ...
  throw new Error(
    [
      `BridgeDesk requires Node ${SUPPORTED_NODE_RANGE}.`,
